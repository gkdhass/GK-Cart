  ...
  if (loading) {
    return (
      <div className="p-6 space-y-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-80 bg-admin-card/80 rounded-2xl border border-white/5 animate-pulse" />
        ))}
      </div>
    );
  }
    ...
    <div className="p-4 lg:p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Overview of store performance</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-admin-card/80 border border-white/5 text-slate-300 text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-admin-accent/30"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Revenue */}
        <div className="bg-admin-card/80 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
          <h3 className="text-white font-semibold text-lg mb-1">Revenue Trend</h3>
          <p className="text-slate-500 text-sm mb-6">Daily revenue over time</p>
          {data?.revenueData?.length > 0 ? (
            ...
            <div className="h-[300px] flex items-center justify-center text-slate-500">No revenue data</div>
          )}
        </div>

        {/* Orders per day */}
        <div className="bg-admin-card/80 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
          <h3 className="text-white font-semibold text-lg mb-1">Orders per Day</h3>
          <p className="text-slate-500 text-sm mb-6">Daily order volume</p>
          {data?.revenueData?.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
            ...
            <div className="h-[300px] flex items-center justify-center text-slate-500">No order data</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-admin-card/80 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
          <div className="flex items-center gap-2 mb-6">
            <HiOutlineTrophy className="w-5 h-5 text-yellow-400" />
            <h3 className="text-white font-semibold text-lg">Top Products</h3>
          </div>
          {data?.topProducts?.length > 0 ? (
            <div className="space-y-3">
              {data.topProducts.slice(0, 8).map((product, idx) => (
                <div key={product._id || idx} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/[0.02] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-slate-500 text-xs">{product.totalSold} sold</p>
                  </div>
        ...
        {/* New Users */}
        <div className="bg-admin-card/80 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
          <h3 className="text-white font-semibold text-lg mb-1">New Users</h3>
          <p className="text-slate-500 text-sm mb-6">Daily user registrations</p>
          {data?.userData?.length > 0 ? (
            ...
            <div className="h-[300px] flex items-center justify-center text-slate-500">No user data</div>
          )}
        </div>
      </div>
      ...
      {data?.categoryData?.length > 0 && (
        <div className="bg-admin-card/80 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
          <h3 className="text-white font-semibold text-lg mb-1">Sales by Category</h3>
          <p className="text-slate-500 text-sm mb-6">Revenue breakdown by product category</p>
          <ResponsiveContainer width="100%" height={300}>
